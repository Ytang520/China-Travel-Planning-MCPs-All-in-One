...
class SimpleOpenSkyTracker:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "FlightTicketMCP/1.0"})
    ...
    def _parse_states_response(
        self, data: Dict, bbox: Optional[tuple] = None
    ) -> Dict[str, Any]:
        """解析OpenSky API响应数据"""
        try:
    ...
    def _parse_state_vector(self, state_array: List) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "on_ground": on_ground,
                "status": status,
                "last_contact": datetime.fromtimestamp(last_contact).isoformat()
                if last_contact
                else None,
                "last_contact_seconds_ago": int(time.time() - last_contact)
                if last_contact
                else None,
            }
    ...
    def get_airport_area_flights(self, airport_code: str) -> Dict[str, Any]:
        ...
        if result.get("status") == "success":
            ...
            result["message"] = (
                f"{airport_code}机场周边找到 {result.get('flight_count', 0)} 架航班"
            )
...
def getAirportFlights(
    airport_code: str, flight_type: str = "departure"
) -> Dict[str, Any]:
    """
    查询机场周边的航班信息
...
def trackMultipleFlights(flight_numbers: List[str], date: str = None) -> Dict[str, Any]:
    ...
    successful_count = sum(
        1
        for r in results
        if r.get("status") == "success" and r.get("flight_count", 0) > 0
    )
