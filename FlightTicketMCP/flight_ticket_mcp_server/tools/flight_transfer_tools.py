...
def getTransferFlightsByThreePlace(
    ...
    查询从出发地通过中转地到目的地的联程航班信息。
     ...
     Args:
         ...
         departure_date (str): 出发日期，默认使用当前日期
    ...
    logger.info(
        f"始发地: {from_place}，中转地：{transfer_place}， 目的地: {to_place}，数据源偏好: {data_source_preference}"
    )
    ...
    try:
        ...
        for trip1 in first_trips:
            ...
            for trip2 in after_trips:
                ...
                if departure_time - arrival_time > timedelta(
                    hours=min_transfer_time
                ) and departure_time - arrival_time < timedelta(
                    hours=max_transfer_time
                ):
                    # 符合换乘时间要求，添加到结果列表
                    logger.info(
                        f"符合换乘时间要求: {trip1.flight_number} {arrival_time} - {trip2.flight_number} {departure_time}"
                    )
                    transfer = FlightTransfer(
                        ...
                        departure_date=departure_date,
                        transfer_time=round(
                            (departure_time - arrival_time).total_seconds() / 3600, 3
                        ),
                    )
                    index += 1
                    logger.info(
                        f"添加中转航班: {transfer.first_flight.flight_number} -> {transfer.second_flight.flight_number}, 中转时间: {transfer.transfer_time}小时"
                    )
        ...
        return {
            "status": "success",
            "message": "查询成功" if select_trips else "未找到符合条件的中转航班",
            "from_place": from_place,
            "transfer_place": transfer_place,
            "to_place": to_place,
            "departure_date": departure_date,
            "transfer_count": len(select_trips),
            "transfers": [transfer.model_dump() for transfer in select_trips],
            "fallback_used": False,
        ...
        logger.warning(
            f"查询中转航班信息失败：{from_place}-{transfer_place}-{to_place}, 错误: {str(e)}",
            exc_info=True,
        )
        primary_error = {
            "status": "error",
            "message": f"查询中转航班失败: {str(e)}",
            "error_code": "TRANSFER_QUERY_FAILED",
            "from_place": from_place,
            "transfer_place": transfer_place,
            "to_place": to_place,
            "requested_data_source": normalized_preference,
...
def _get_location_code(place: str) -> str:
    ...
    try:
        ...
        if len(results) == 0:
            ...
            text = results[0].text
            code_array = [line.strip().split() for line in text.strip().splitlines()][
                1:
            ]
...
def _get_location_codev2(place: str) -> str:
    ...
    try:
        ...
        # 百度汉语的输入框ID是kw
        input_box = search_box.find_element(
            By.NAME, "keyword"
        )  # 百度汉语的输入框ID是kw
        input_box.clear()
        input_box.send_keys(place)
...
def _get_direct_airline(from_code: str, to_code: str) -> Optional[List[Flight]]:
    """
    :param from_code:
    :param to_code:
    :return:
